...
<?php               
        ...
        if(isset($_POST['book_btn2']))
        {

            if($_POST['datetime']=='')
            {
                $_SESSION['book']="Please Select Date";
                header('Location: movie2.php');
            }
            elseif($_POST['seat_no']=='')
            {
                $_SESSION['book']="Please Select Seat";
                header('Location: movie2.php');
            }
            elseif($_POST['datetime']!=='' && $_POST['seat_no']!=='')
            {
                $userid=$_POST['user'];            
                $name=$_POST['mname'];
                $d=$_POST['datetime'];
                ...
                $sql1="SELECT * FROM booking WHERE ticket_date_time='$d'";
                $sql1_run=mysqli_query($connection,$sql1);

                if($rowS1=mysqli_fetch_assoc($sql1_run))
                {
                    $selectid=$rowS1['show_id'];
                    $sql2="SELECT * FROM seat_detail WHERE show_id='$selectid'";
                    $sql2_run=mysqli_query($connection,$sql2);

                    while($rowS2=mysqli_fetch_assoc($sql2_run))
                    {

                        $selectSeat=$rowS2['seat_name'];
                        $selectSeat2=explode(' ',$selectSeat);

                        if( $selectSeat2 === $seat)
                        {
                            $_SESSION['book']="Seat Already Selected";
                            header('Location: movie2.php');
                        }
                        else
                        {
                            ...
                            $_SESSION['movie']=$name;
                            $_SESSION['show_dtime']=$d;
                            $_SESSION['no_seat']=$count;
                            $_SESSION['seat_name']=$seat1;
                            $_SESSION['amount']=$total;
                            foreach($seat as $seat2){
                                $query="INSERT INTO seat_reserved (user_id,show_id,seat_name,reserved)
                                VALUE('$userid','$selectid','$seat2','true')";
                                $query_run=mysqli_query($connection,$query);
                                }
...
?>
        ...
        if(isset($_POST['book_btn']))
        {
            ...
            else
            {
                ...
                VALUES ('$ticketid','$userid','$show_id','$seat1','$name','$count','$d','$total','$dd')";
                $query1_run= mysqli_query($connection,$query1);
                    ...
                    $query3="INSERT INTO payment(user_id,show_id,booking_id,name,email,card_no,ex_date,cvv,add_date)
                    VALUES ('$userid','$show_id','$bookid2','$first_name','$email','$card_no','$ex_date','$cvv','$dd')";
